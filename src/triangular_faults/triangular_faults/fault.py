...
from triangular_faults.triangular_disp_csi import displacement
...
class RsqSimSegment:
    ...
    def patch_numbers(self):
        if self.patch_outlines is not None:
            return np.array([patch.patch_number for patch in self.patch_outlines])
        else:
            return None
    ...
    def triangles(self):
        if self._triangles is None:
            self.collect_triangles()
        return self._triangles

    def collect_triangles(self):
        if any([a is None for a in [self.vertices, self.patch_vertices]]):
            raise ValueError("Need to read in triangles first!")
        triangle_ls = []
        for patch in self.patch_vertices:
            patch_vertex_ls = []
            for vertex in patch:
                patch_index = np.where((self.vertices == vertex).all(axis=1))[0][0]
                patch_vertex_ls.append(patch_index)


            triangle_ls.append(patch_vertex_ls)
    ...
    def from_triangles(cls, triangles: Union[np.ndarray, list, tuple], segment_number: int = 0,
        ...
        assert isinstance(segment_number, int)
        if patch_numbers is None:
            patch_numbers = np.arange(len(triangle_array))
